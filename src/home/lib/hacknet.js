...
export class Network {
	...
	buyUpgrades(budget, payoffTime=Infinity) {
		const upgrades = this.getServerUpgrades(budget, payoffTime);
	...
	getServerUpgrades = (budget, payoffTime) => this.servers
		.flatMap(server => server.upgradeInfos)
		.filter(upgrade => budget > upgrade.price())
		.filter(upgrade => upgrade.price() / moneyPerSecond(upgrade.hashRateIncrease()) < payoffTime)
