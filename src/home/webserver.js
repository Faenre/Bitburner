...
export async function main(ns) {
	ns.tail();
	loadAdditionalRam(ns);

	const server = new Server(ns);
	await server.listen();
}

// @TODO: load only the scripts that are "actually" loaded
function loadAdditionalRam(ns) {
	ns.ramOverride((
		(scripts) =>
			ns.getScriptRam(ns.getScriptName())
			+ scripts.reduce((sum, s) => sum + ns.getScriptRam(s) - 1.60, 0.00)
	)(ns.ls('home', 'endpoints/')));
}
